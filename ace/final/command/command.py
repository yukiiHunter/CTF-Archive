...
def pad(data):
    ...
    assert len(data) <= 16
    if len(data) != 16: data += b'\x01' + b'\x00' * (15 - len(data))
    return data
...
class CAAS:
    ...
    def encrypt(self, nonce, plaintext):
        ...
        assert len(nonce) == 16
        delta = int(len(plaintext) % 16).to_bytes(16, byteorder='little')
        Kn, _ = aes_encrypt(self.key, nonce)
        T, K = aes_encrypt(Kn, nonce)
        ciphertext = b''
        for i in range(0, len(plaintext), 16):
            T, block = self.enc_tag(T, plaintext[i:i+16])
            ciphertext += block
            T, K = aes_encrypt(K, T)
    ...
    def decrypt(self, nonce, ciphertext, tag):
        ...
        assert len(nonce) == 16
        delta = int(len(ciphertext) % 16).to_bytes(16, byteorder='little')
        Kn, _ = aes_encrypt(self.key, nonce)
        T, K = aes_encrypt(Kn, nonce)
        plaintext = b''
        for i in range(0, len(ciphertext), 16):
            T, block = self.dec_tag(T, ciphertext[i:i+16])
            #print(block)
            plaintext += block
            T, K = aes_encrypt(K, T)
